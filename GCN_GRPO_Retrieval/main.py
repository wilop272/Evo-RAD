...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='retizero', choices=['eyeclip', 'retizero'])
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            # We need to collect ALL inputs for global metric computation (AUC requires all samples)
            all_logits_hard = []
            all_logits_soft = []
            all_targets = []
            ...
            print(f"  Hard Vote: ACC={metrics_hard['ACC']:.2f}, F1={metrics_hard['F1']:.2f}, AUC={metrics_hard['AUC']:.2f}")
            print(f"  Soft Vote: ACC={metrics_soft['ACC']:.2f}, F1={metrics_soft['F1']:.2f}, AUC={metrics_soft['AUC']:.2f}")
    ...
    print(f"  AUC: {test_metrics_hard['AUC']:.2f}")
    print(f"  Sensitivity: {test_metrics_hard['Sensitivity']:.2f}")
    print(f"  Specificity: {test_metrics_hard['Specificity']:.2f}")
    print(f"\nSoft Vote:")
    print(f"  ACC: {test_metrics_soft['ACC']:.2f}")
    print(f"  F1: {test_metrics_soft['F1']:.2f}")
    print(f"  AUC: {test_metrics_soft['AUC']:.2f}")
    print(f"  Sensitivity: {test_metrics_soft['Sensitivity']:.2f}")
    print(f"  Specificity: {test_metrics_soft['Specificity']:.2f}")
